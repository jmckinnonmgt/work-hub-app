...
export function LearnList({ tasks, onOpen }: { tasks: Task[]; onOpen?: (task: Task) => void }) {
  return (
    <div style={{ maxWidth: 720, margin: "20px 22px" }}>
      ...
      {tasks.map((t) => (
        <div key={t.itemId} onClick={() => onOpen?.(t)} style={{ display: "flex", alignItems: "baseline", gap: 12, padding: "15px 4px", borderBottom: `1px solid ${tokens.line}`, cursor: "pointer" }}>
