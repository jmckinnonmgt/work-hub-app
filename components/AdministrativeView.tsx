...
export function AdministrativeView({ tasks, onOpen }: { tasks: Task[]; onOpen?: (task: Task) => void }) {
  return (
    <div style={{ maxWidth: 720, margin: "20px 22px", display: "flex", flexDirection: "column", gap: 9 }}>
      {tasks.map((m) => (
        <div key={m.itemId} onClick={() => onOpen?.(m)} style={{ display: "flex", alignItems: "center", gap: 12, border: `1px solid ${tokens.line}`, background: tokens.panel, borderRadius: 7, padding: "13px 15px", cursor: "pointer" }}>
          <div style={{ flex: 1 }}>
