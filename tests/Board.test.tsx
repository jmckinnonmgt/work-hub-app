...
const mk = (id: string, column: Task["column"]): Task => ({
  ...
  category: "Build", source: "Self", column, repo: "", branch: "",
});

describe("Board", () => {
  it("renders four column headers", () => {
    render(<Board tasks={[mk("a", "next")]} onMove={vi.fn()} />);
    for (const label of ["Next", "In progress", "Blocked", "Done"]) {
      expect(screen.getByText(label)).toBeInTheDocument();
    }
  });
  it("calls onMove when a card is dropped on a column", () => {
    const onMove = vi.fn();
    render(<Board tasks={[mk("a", "next")]} onMove={onMove} />);
